program ising_model
	...
	double precision :: coupling_constant, h, rand, kt, r, energy_difference, magnetization, sum_of_ensemble, ensemble_average, sum_of_some_E, E, std
	integer , parameter :: rep=10000000, num_of_ensemble=100, bin_size=10000
	integer, dimension(:,:), allocatable :: lattice
	double precision, dimension(:), allocatable :: mean_of_E, M_of_ensemble, square_sum
	open(1, file='input', status='old')   ! input of lattice_size, J, h
	open(2, file='/home/jina/monte_carlo/final/data/50lattice.dat', status='unknown', position='append')
	...
	allocate(square_sum(num_of_ensemble))
...
100	      do i=1, lattice_size
		 ...
		 if (mod(i,5000000)==0) print*, i, 'th M for equilibration: ', magnetization
	         endif
...
!	            print*, i, '까지의 평균 E: ', mean_of_E(i/bin_size)
	            ...
	            if (i .ne. bin_size) then
	               if (abs(mean_of_E(i/bin_size)-mean_of_E(i/bin_size-1))<0.1d0) then
...
10	      print*, p, 'th ensemble의 M: ', magnetization
	      ...
	      square_sum(p)=magnetization*magnetization
	
           enddo
	   
	   ensemble_average=sum(M_of_ensemble)/(dble(num_of_ensemble))
	   std=sqrt(sum(square_sum)/dble(num_of_ensemble)-ensemble_average)
	  

           !---------------------------------------------------------------------------------
           ! End of trials

           print*, 'kt: ', kt,  'M: ', ensemble_average, 'std: ', std
